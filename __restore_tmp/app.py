...
def serve_video_file_with_range_support(file_path, is_cached='false'):
    ...
    range_header = request.headers.get('Range')
    # Proper HEAD handling: return only headers with 200 (or 206 if range specified) to avoid misleading partial responses
    if request.method == 'HEAD':
        # If client sent a Range with HEAD, we still answer 200 with full length (simpler & more compatible)
        ...
        print(f"DEBUG: slice-video HEAD response size={file_size} path={file_path}")
        return resp
...
def generate_video_slice_ffmpeg(input_path, output_path, mode, count, order):
    ...
    try:
        ...
        if result.returncode == 0:
            try:
                out_size_mb = os.path.getsize(output_path) / 1024 / 1024.0
                # Quick probe of output for integrity metrics (width/height/duration)
                try:
                    probe_out = subprocess.run(probe_cmd[:-1] + [output_path], capture_output=True, text=True, timeout=20)
                    if probe_out.returncode == 0:
                        import json as _json
                        pdata = _json.loads(probe_out.stdout)
                        o_vid = None
                        o_dur = None
                        for s in pdata.get('streams', []):
                            if s.get('codec_type') == 'video' and o_vid is None:
                                o_vid = s
                        fmt = pdata.get('format') or {}
                        o_dur = fmt.get('duration')
                        if o_vid:
                            ow = o_vid.get('width'); oh = o_vid.get('height')
                            print(f"DEBUG: Slice integrity width={ow} height={oh} duration={o_dur} size_mb={out_size_mb:.2f} has_audio={has_audio}")
                        else:
                            print(f"DEBUG: Slice integrity (no video stream parsed) size_mb={out_size_mb:.2f}")
                    else:
                        print(f"WARNING: Could not probe output slice: {probe_out.stderr[:120]}")
                except Exception as _probeE:
                    print(f"WARNING: Exception probing output slice: {_probeE}")
                print(f"DEBUG: FFmpeg succeeded, output file size: {out_size_mb:.2f} MB")
            except Exception as _szE:
                print(f"DEBUG: FFmpeg succeeded (size check failed: {_szE})")
            return True
        else:
