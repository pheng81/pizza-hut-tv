#!/usr/bin/env python3
"""
Pizza Hut TV - Pi GUI Client (FINAL WORKING VERSION)
Uses the correct video serving URL structure
"""
...
class PizzaHutTVApp:
    ...
    def create_playing_step(self):
        ...
        tk.Button(button_frame, text='Use HLS Stream',
                         command=self.start_hls_playback,
                         bg='#c8102e',
                         fg='white',
                         font=('Arial', 14, 'bold'),
                         relief='flat',
        ...
        tk.Button(button_frame, text='Try Static URL',
                         command=self.start_static_playback,
                         bg='#c8102e',
                         fg='white',
                         font=('Arial', 12, 'bold'),
                         relief='flat',
    ...
    def get_screen_info(self):
        """Get screen info from the nested structure"""
        # Look in the store's screens
        if str(self.store_code) in self.screen_data:
            store_screens = self.screen_data[str(self.store_code)]
            screen_key = f"{self.store_code}_screen{self.screen_id}"
            if screen_key in store_screens:
                screen_info = store_screens[screen_key]
    ...
    def start_hls_playback(self):
        """Use the HLS stream endpoint that works with webplayer"""
        hls_url = f'{self.server_url}/stream/{self.store_code}/{self.screen_id}/playlist.m3u8'
        messagebox.showinfo('Starting HLS Playback', f'Playing HLS stream:\n{hls_url}\n\nThis uses the same streaming method as the webplayer.')
        
        try:
            subprocess.Popen(['vlc', '--intf', 'dummy', '--fullscreen', hls_url])
        except FileNotFoundError:
            messagebox.showerror('VLC Not Found', 'VLC media player is not installed.\n\nInstall with:\nsudo apt install vlc')
    
    def start_static_playback(self):
        """Try different static file URL patterns"""
        video_file, screen_info = self.get_screen_info()
        
        if video_file and screen_info:
            # Try multiple URL patterns
            possible_urls = [
                f'{self.server_url}/static/{video_file}',
                f'{self.server_url}/uploads/{video_file}',
                f'{self.server_url}/media/{video_file}',
                f'{self.server_url}/files/{video_file}',
                f'{self.server_url}/{video_file}'
            ]
            
            messagebox.showinfo('Trying Static URLs', f'Will try these URLs:\n\n' + '\n'.join(possible_urls[:3]) + '\n\n(VLC will try until one works)')
            
            # Start VLC with the first URL (it will show errors but might work)
            try:
                subprocess.Popen(['vlc', '--intf', 'dummy', '--fullscreen', possible_urls[0]])
            except FileNotFoundError:
                messagebox.showerror('VLC Not Found', 'VLC media player is not installed.\n\nInstall with:\nsudo apt install vlc')
        else:
            messagebox.showerror('No Video Found', 'No video file information available')
