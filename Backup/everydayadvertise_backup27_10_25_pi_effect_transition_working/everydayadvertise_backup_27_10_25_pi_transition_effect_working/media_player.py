...
class MediaPlayer:
    ...
    def __init__(self, screen: pygame.Surface, cache_dir: str = None):
        ...
        # Robust transition guard: ensure only one transition runs at a time and allow cancellation
        self._active_transition_id = 0
    ...
    def _begin_transition(self) -> int:
        """Start a new transition and cancel any previous one."""
        self._active_transition_id = (self._active_transition_id + 1) % 1_000_000_000
        self._transition_active = True
        return self._active_transition_id

    def _cancel_transitions(self):
        """Cancel any in-flight transition immediately."""
        self._active_transition_id = (self._active_transition_id + 1) % 1_000_000_000
        self._transition_active = False

    def _is_transition_current(self, tid: int) -> bool:
        return tid == self._active_transition_id

    def _end_transition(self, tid: int):
        if tid == self._active_transition_id:
            self._transition_active = False
    ...
    def play_media(self, url: str, effect: str = 'fade', duration: float = 10.0) -> bool:
        ...
        try:
            ...
            if media_type == 'video' and self.video_player:
                ...
                # Cancel any in-flight surface transitions before switching to external video
                self._cancel_transitions()
                ...
                tid = self._begin_transition()
                success = transition_func(surface, duration, tid=tid)
    ...
    def _fade_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
        ...
        try:
            ...
            for i in range(steps + 1):
                if not self._is_transition_current(tid):
                    # Transition superseded; draw final state and exit
    ...
    def _slide_left_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
    ...
    def _slide_right_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
    ...
    def _zoom_in_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
    ...
    def _zoom_out_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
    ...
    def _cut_transition(self, new_surface: pygame.Surface, duration: float, tid: int = None) -> bool:
        """Instant cut transition (no animation)."""
        try:
            # Cancel any previous transition and start a new one then end immediately
            if tid is None:
                tid = self._begin_transition()
            self.screen.blit(new_surface, (0, 0))
            pygame.display.flip()
