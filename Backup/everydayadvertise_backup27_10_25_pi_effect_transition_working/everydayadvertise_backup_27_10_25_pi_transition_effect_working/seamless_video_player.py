...
class SeamlessMediaPlayer:
    ...
    def play_media(self, url: str, effect: str, duration: float, item: Optional[dict] = None, **_ignored) -> bool:
        ...
        try:
            ...
            if media_type == 'video' and self.current_media_type == 'video':
                if self.v2v_transitions and effect and effect.lower() != 'cut':
                    ...
                    try:
                        ...
                        if isinstance(sref, dict) and sref.get('start_epoch') and hasattr(self.video_player, 'time_sync') and self.video_player.time_sync:
            ...
            if media_type == 'image':
                # If we are coming from video, pause MPV and grab the current frame
                from_surface = None
                if self.current_media_type == 'video':
                    try:
                        if self.video_player and self.video_player.player:
                            self.video_player.player.pause = True
                            logger.info("⏸️  Paused MPV to transition into image")
                    except Exception as e:
                        logger.warning(f"⚠️  Could not pause MPV before image transition: {e}")
