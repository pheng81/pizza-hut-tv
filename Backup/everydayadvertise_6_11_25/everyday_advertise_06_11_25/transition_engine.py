...
class TransitionEngine:
    def __init__(self, screen):
        ...
        # Transition guard: ensure only one transition runs at a time
    ...
    def begin_transition(self) -> int:
        """Start a new transition and cancel any existing animation loops."""
        self._transition_id = (self._transition_id + 1) % 1_000_000_000
        return self._transition_id

    def cancel_transitions(self):
        """Invalidate any in-flight transitions immediately."""
        self._transition_id = (self._transition_id + 1) % 1_000_000_000

    def _is_current(self, tid: int) -> bool:
        return tid == self._transition_id
    ...
    def _animate(self, render_fn, fps: Optional[int] = None, tid: Optional[int] = None):
        ...
        while True:
            ...
            try:
                ...
                pass
            # Abort early if a new transition superseded this one
            if tid is not None and not self._is_current(tid):
                break
            try:
                render_fn(off, t)
                self._present(off)
    ...
    def apply_transition(self, from_surface, to_surface, effect, tid: Optional[int] = None):
        if not to_surface:
            return False
        ...
        try:
            ...
            elif e == "fade": self._fade(from_surface, to_surface, tid=tid)
            elif e == "dissolve": self._dissolve(from_surface, to_surface, tid=tid)
            elif e == "slide_left": self._slide_left(from_surface, to_surface, tid=tid)
            elif e == "slide_right": self._slide_right(from_surface, to_surface, tid=tid)
            elif e == "slide_up": self._slide_up(from_surface, to_surface, tid=tid)
            elif e == "slide_down": self._slide_down(from_surface, to_surface, tid=tid)
            elif e == "zoom_in": self._zoom_in(from_surface, to_surface, tid=tid)
            elif e == "zoom_out": self._zoom_out(from_surface, to_surface, tid=tid)
            elif e == "wipe": self._wipe(from_surface, to_surface, tid=tid)
    ...
    def _fade(self, from_surface, to_surface, tid: Optional[int] = None):
    ...
    def _dissolve(self, from_surface, to_surface, tid: Optional[int] = None):
        from_scaled = self._prep_surface(from_surface)
        to_scaled = self._prep_surface(to_surface)
        from_small = self._downscale(from_scaled) if from_scaled else None
        to_small = self._downscale(to_scaled)
    ...
    def _slide_left(self, from_surface, to_surface, tid: Optional[int] = None):
    ...
    def _slide_right(self, from_surface, to_surface, tid: Optional[int] = None):
        bg = self._downscale(self._prep_surface(from_surface)) if from_surface else None
        to_small = self._downscale(self._prep_surface(to_surface))
        off_w, off_h = self._offscreen_size()
    ...
    def _slide_up(self, from_surface, to_surface, tid: Optional[int] = None):
        bg = self._downscale(self._prep_surface(from_surface)) if from_surface else None
        to_small = self._downscale(self._prep_surface(to_surface))
        off_w, off_h = self._offscreen_size()
    ...
    def _slide_down(self, from_surface, to_surface, tid: Optional[int] = None):
        bg = self._downscale(self._prep_surface(from_surface)) if from_surface else None
        to_small = self._downscale(self._prep_surface(to_surface))
        off_w, off_h = self._offscreen_size()
    ...
    def _zoom_in(self, from_surface, to_surface, tid: Optional[int] = None):
        ...
        self._animate(render, fps=min(self.fps, 24), tid=tid)
        # Final frame
        self.screen.fill((0, 0, 0))
        self.screen.blit(to_surface, (0, 0))
        try:
    ...
    def _zoom_out(self, from_surface, to_surface, tid: Optional[int] = None):
    ...
    def _wipe(self, from_surface, to_surface, tid: Optional[int] = None):
