...
class EATVWebplayerClone:
    def __init__(self, root):
        ...
        # Emergency Exit System - Multiple Methods with Focus Management
        self.root.bind('<KeyPress>', self.handle_keypress)
        self.root.bind('<Key>', self.handle_keypress)  # Alternative key binding
        self.root.bind('<Button-1>', self.regain_focus)  # Click to regain focus
        self.root.focus_set()
        self.root.focus_force()
        
        # Ensure keyboard focus is maintained
        self.root.after(1000, self.maintain_keyboard_focus)
        self.root.after(5000, self.maintain_keyboard_focus)  # Backup focus check
        ...
        # Freeze detection and recovery system
        self.last_activity_time = time.time()
        self.vlc_start_time = None
        self.freeze_check_counter = 0
        self.max_vlc_processes = 8  # Maximum VLC processes for digital signage (increased from 3)
        self.last_content_change = time.time()
        self.recovery_attempt_count = 0
        self.max_recovery_attempts = 3
        
        # HTTP session for connection reuse and optimization
        self.session = requests.Session()
        self.session.headers.update({
            'User-Agent': 'EA-TV-Pi-Client/1.0 (Fast-Loading)',
            'Accept': 'application/json, video/mp4, image/*, */*',
        ...
        # Start schedule monitoring timer - check every 10 seconds for faster dashboard updates
        self.root.after(5000, self.schedule_monitor)
        
        # Start freeze detection monitoring - check every 10 minutes for digital signage
        self.root.after(600000, self.check_for_freeze)  # Start after 10 minutes
    ...
    def create_widgets(self):
        ...
        self.code_entry.bind('<FocusIn>', self.on_entry_focus_in)
        self.code_entry.bind('<FocusOut>', self.on_entry_focus_out)
        ...
        self.store_entry.bind('<FocusIn>', self.on_entry_focus_in)
        self.store_entry.bind('<FocusOut>', self.on_entry_focus_out)
    ...
    def show_step(self):
        ...
        if self.current_step == 1:
            ...
            self.root.after(100, lambda: self.set_entry_focus(self.code_entry))
        elif self.current_step == 2:
            self.step2_frame.pack(fill='both', expand=True)
            self.root.after(100, lambda: self.set_entry_focus(self.store_entry))
        elif self.current_step == 3:
            self.step3_frame.pack(fill='both', expand=True)
        elif self.current_step == 4:
            self.step4_frame.pack(fill='both', expand=True)
    
    def set_entry_focus(self, entry_widget):
        """Helper method to properly set focus on Entry widgets"""
        try:
            entry_widget.focus_set()
            entry_widget.focus_force()
            entry_widget.icursor(tk.END)  # Place cursor at end
            print(f"DEBUG: Focus set on entry widget: {entry_widget}")
        except Exception as e:
            print(f"DEBUG: Error setting entry focus: {e}")
    ...
    def play_video(self):
        ...
        try:
            # SAME AS WEBPLAYER: Use playlist endpoint with X-User-Code header and cache-busting
            headers = {
            ...
            playlist_url = f"{self.server_url}/playlist/{self.store_id}/{self.screen_id}?_t={cache_buster}"
...
'''
    ...
    def handle_keypress(self, event):
        ...
        print(f"DEBUG: Key pressed: {key} (focus: {focused_widget})")
        
        # Don't intercept keys if an Entry widget has focus
        if isinstance(focused_widget, tk.Entry):
            print(f"DEBUG: Entry widget has focus, allowing normal input")
            return
        ...
        # Space to toggle status or regain control
        elif key == 'space':
            print("DEBUG: Space pressed - regaining focus and showing status")
            self.regain_focus()
            if hasattr(self, 'status_label'):
                print(f"DEBUG: Current status: {self.status_label.cget('text')}")
    
    def regain_focus(self, event=None):
        """Regain keyboard focus from VLC"""
        try:
            print("DEBUG: Attempting to regain keyboard focus...")
            self.root.lift()
            self.root.focus_force()
            self.root.focus_set()
            
            # Make window briefly visible to regain focus
            self.root.deiconify()
            self.root.update()
            
            print("DEBUG: Keyboard focus regained")
        except Exception as e:
            print(f"DEBUG: Error regaining focus: {e}")
    
    def maintain_keyboard_focus(self):
        """Periodically ensure the application maintains keyboard focus"""
        try:
            # Check if we have focus
            current_focus = self.root.focus_get()
            
            # Don't interfere if an Entry widget has focus
            if isinstance(current_focus, tk.Entry):
                print(f"DEBUG: Entry widget has focus, not interfering: {current_focus}")
            elif current_focus != self.root:
                print(f"DEBUG: Focus lost (current: {current_focus}) - attempting to regain")
                self.regain_focus()
            
            # Schedule next focus check
            self.root.after(10000, self.maintain_keyboard_focus)  # Check every 10 seconds
            
        except Exception as e:
            print(f"DEBUG: Error in focus maintenance: {e}")
            # Still schedule next check
            self.root.after(10000, self.maintain_keyboard_focus)
    
    def on_entry_focus_in(self, event):
        """Handle when Entry widget gains focus - ensure proper input handling"""
        print(f"DEBUG: Entry widget gained focus: {event.widget}")
        # Make sure the entry widget can receive keyboard input
        event.widget.focus_set()
        
    def on_entry_focus_out(self, event):
        """Handle when Entry widget loses focus"""
        print(f"DEBUG: Entry widget lost focus: {event.widget}")
    ...
    def schedule_monitor(self):
        ...
        try:
            ...
            cache_buster = int(time.time())
            playlist_url = f"{self.server_url}/playlist/{self.store_id}/{self.screen_id}?_t={cache_buster}"
            response = self.session.get(playlist_url, headers=headers, timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                playlist = data.get('playlist', [])
                
                # PRELOADING OPTIMIZATION: Start preloading next video in background
                self.preload_next_videos(playlist)
        ...
        # Schedule next check in 10 seconds for faster dashboard updates
        self.root.after(10000, self.schedule_monitor)

    def preload_next_videos(self, playlist):
        """Preload upcoming videos to make transitions near-instant"""
        try:
            if not playlist:
                return
                
            # Find what should be playing in the next 5 minutes
            ...
            for item in playlist:
                ...
                # Get item schedule info
                start_time = self.parse_time_to_seconds(item.get('start', '00:00'))
                end_time = self.parse_time_to_seconds(item.get('end', '23:59'))
                
                # Check if item will be playing soon (within 5 minutes)
                current_seconds = (current_time % 86400)  # Seconds since midnight
                
                if self.is_time_in_range(current_seconds, start_time, end_time):
                    upcoming_items.append(item)
                    
            # Preload first few upcoming videos (max 2 to avoid resource exhaustion)
            for i, item in enumerate(upcoming_items[:2]):
                if i == 0:
                    continue  # Skip current item
                    
                video_url = item.get('url')
                if video_url and 'slice-video' in video_url:
                    print(f"DEBUG: 🚀 Preloading slice video: {item.get('file')}")
                    # Start background preload process
                    self.start_background_preload(video_url)
                    
        except Exception as e:
            print(f"DEBUG: Preload error: {e}")
    
    def start_background_preload(self, video_url):
        """Start background VLC process to preload/cache video with parallel loading for slices"""
        try:
            # For slice videos, preload all slices in parallel for instant switching
            if 'slice-video' in video_url and 'slice_count=' in video_url:
                self.preload_all_slices(video_url)
            else:
                # Regular video preload
                self.preload_single_video(video_url)
                
        except Exception as e:
            print(f"DEBUG: Preload process error: {e}")
    
    def preload_all_slices(self, base_slice_url):
        """Preload all slices of a video in parallel for instant multi-screen switching"""
        try:
            # Extract slice parameters
            if 'slice_count=' not in base_slice_url:
                return
                
            # Get slice count from URL
            import re
            slice_count_match = re.search(r'slice_count=(\d+)', base_slice_url)
            if not slice_count_match:
                return
                
            slice_count = int(slice_count_match.group(1))
            print(f"DEBUG: 🚀 Parallel preloading {slice_count} slices")
            
            # Create preload processes for all slices
            for slice_order in range(slice_count):
                slice_url = re.sub(r'slice_order=\d+', f'slice_order={slice_order}', base_slice_url)
                
                preload_args = [
                    'vlc',
                    '--intf', 'dummy',
                    '--no-video',           # Don't display
                    '--no-audio',           # Don't play audio
                    '--run-time=2',         # Quick 2-second preload
                    '--network-caching=15000',  # Extra large cache for slices
                    '--file-caching=8000',
                    '--stop-time=2',
                    slice_url,
                    'vlc://quit'
                ...
                # Start each slice preload in parallel
                subprocess.Popen(preload_args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                print(f"DEBUG: Preloading slice {slice_order}: {slice_url}")
                
        except Exception as e:
            print(f"DEBUG: Parallel slice preload error: {e}")
    
    def preload_single_video(self, video_url):
        """Preload a single video file"""
        preload_args = [
            'vlc',
            '--intf', 'dummy',
            '--no-video',           # Don't display video
            '--no-audio',           # Don't play audio  
            '--run-time=3',         # Run for 3 seconds to buffer
            '--network-caching=10000',  # Large cache
            '--file-caching=5000',
            '--stop-time=3',        # Stop after 3 seconds
            video_url,
            'vlc://quit'
        ]
        
        # Start preload process in background
        subprocess.Popen(preload_args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print(f"DEBUG: Background preload started for: {video_url}")
    ...
    def start_optimized_content(self, video_item):
        ...
        try:
            ...
            # Check if this is a slice video and apply specialized handling
            if self.is_slice_video(video_item):
                print("DEBUG: Detected slice video - applying specialized VLC args")
                vlc_args = self.get_slice_video_args(base_args, video_url)
                # Start slice video health monitoring
                self.root.after(30000, self.monitor_slice_video_health)
            else:
                # Apply comprehensive fullscreen settings for regular videos
            ...
            try:
                ...
                print(f"DEBUG: VLC startup failed: {e}")
                if self.is_slice_video(video_item):
                    print("DEBUG: Slice video startup failed - attempting recovery")
                    self.handle_slice_video_failure(video_item, f"VLC startup error: {e}")
                    return
                else:
                    raise
            
            # Track VLC start time for freeze detection
            self.vlc_start_time = time.time()
            self.last_activity_time = time.time()
            self.last_content_change = time.time()
    ...
    def check_for_freeze(self):
        """Detect if the player is frozen and initiate recovery"""
        current_time = time.time()
        
        # Update activity time when we detect legitimate activity
        if self.vlc_process and self.vlc_process.poll() is None:
            # VLC is running - this is activity
            self.last_activity_time = current_time
            
        # Check for various freeze conditions - DIGITAL SIGNAGE TIMEOUTS
        freeze_detected = False
        freeze_reason = ""
        
        # Condition 1: No activity for too long (2 hours for digital signage)
        if current_time - self.last_activity_time > 7200:  # 2 hours
            freeze_detected = True
            freeze_reason = f"No activity for {int(current_time - self.last_activity_time)}s"
        
        # Condition 2: VLC running too long without natural end (4 hours for digital signage)
        elif self.vlc_process and self.vlc_start_time and (current_time - self.vlc_start_time) > 14400:  # 4 hours
            freeze_detected = True
            freeze_reason = f"VLC running too long ({int(current_time - self.vlc_start_time)}s)"
        
        # Condition 3: Same content playing too long without schedule change (6 hours for digital signage)
        elif (current_time - self.last_content_change) > 21600:  # 6 hours
            freeze_detected = True
            freeze_reason = f"No content change for {int(current_time - self.last_content_change)}s"
        
        # Condition 4: Check for too many VLC processes (resource leak)
        try:
            vlc_count = int(subprocess.check_output(['pgrep', '-c', 'vlc']).decode().strip())
            if vlc_count > self.max_vlc_processes:
                freeze_detected = True
                freeze_reason = f"Too many VLC processes ({vlc_count})"
        except:
            pass
        
        if freeze_detected:
            print(f"DEBUG: FREEZE DETECTED - {freeze_reason}")
            self.initiate_recovery(freeze_reason)
        else:
            # No freeze - schedule next check in 10 minutes (digital signage interval)
            self.root.after(600000, self.check_for_freeze)
    
    def initiate_recovery(self, reason):
        """Initiate recovery from a frozen state"""
        print(f"DEBUG: ===== INITIATING RECOVERY =====")
        print(f"DEBUG: Recovery reason: {reason}")
        print(f"DEBUG: Attempt #{self.recovery_attempt_count + 1}")
        
        if self.recovery_attempt_count >= self.max_recovery_attempts:
            print("DEBUG: Maximum recovery attempts reached - performing full restart")
            self.perform_full_restart()
            return
            
        self.recovery_attempt_count += 1
        
        try:
            # Step 1: Emergency stop all VLC processes
            self.emergency_stop()
            
            # Step 2: Clean up any zombie processes
            ...
            # Step 5: Restart content playback
            if hasattr(self, 'status_label'):
                self.status_label.config(text="🔄 Recovering from freeze...", fg='#FF6600')
                
            print("DEBUG: Attempting to restart content after recovery")
            self.root.after(3000, self.restart_content_after_recovery)
    ...
    def cleanup_zombie_processes(self):
        """Clean up any zombie VLC processes"""
        try:
            print("DEBUG: Cleaning up zombie processes...")
            
            # Kill all VLC processes more aggressively
            subprocess.run(['pkill', '-9', 'vlc'], timeout=5, capture_output=True)
            subprocess.run(['killall', '-9', 'vlc'], timeout=5, capture_output=True)
            
            # Clean up any stuck X11 processes
            subprocess.run(['pkill', '-9', 'Xvlc'], timeout=5, capture_output=True)
            
            print("DEBUG: Zombie process cleanup completed")
            
        except Exception as e:
            print(f"DEBUG: Error during zombie cleanup: {e}")
    
    def reset_internal_state(self):
        """Reset internal variables to clean state"""
        print("DEBUG: Resetting internal state...")
        ...
        if hasattr(self, 'emergency_timer') and self.emergency_timer:
            try:
                self.root.after_cancel(self.emergency_timer)
            except:
                pass
            self.emergency_timer = None
        
        print("DEBUG: Internal state reset completed")
    
    def restart_content_after_recovery(self):
        """Restart content playback after recovery"""
        try:
            print("DEBUG: Restarting content after recovery...")
            
            if hasattr(self, 'status_label'):
                self.status_label.config(text="🔄 Restarting content...", fg='#90EE90')
            
            # Try to get current schedule and start fresh
            self.auto_play_next_scheduled()
            
            # Reset recovery counter on successful restart
            self.recovery_attempt_count = 0
            
            # Resume freeze monitoring with 10-minute intervals for digital signage
            self.root.after(600000, self.check_for_freeze)
            
        except Exception as e:
            print(f"DEBUG: Failed to restart content after recovery: {e}")
            self.perform_full_restart()
    
    def perform_full_restart(self):
        """Perform a full application restart as last resort"""
        print("DEBUG: ===== PERFORMING FULL RESTART =====")
        
        try:
            if hasattr(self, 'status_label'):
                self.status_label.config(text="⚠️ Full restart required", fg='#FF0000')
            ...
            print(f"DEBUG: Full restart failed: {e}")
            print("DEBUG: Manual intervention required")
            if hasattr(self, 'status_label'):
                self.status_label.config(text="❌ Manual restart required", fg='#FF0000')
    ...
    def auto_play_next_scheduled(self):
        ...
        try:
            ...
            # Refresh playlist to get current scheduled items with cache-busting
            headers = {
                'X-User-Code': self.code,
                'User-Agent': 'EA-TV-Pi-Client/1.0',
                'Cache-Control': 'no-cache, no-store, must-revalidate',
    ...
    def get_comprehensive_fullscreen_args(self, base_args, video_url):
        ...
        return base_args + [
            ...
            '--no-keyboard-events',  # CRITICAL: Disable VLC keyboard capture
            '--no-mouse-events',     # CRITICAL: Disable VLC mouse capture
            '--no-interact',         # Disable VLC interactive interface
            '--no-stats',            # Disable statistics interface
    ...
    def is_slice_video(self, video_item):
        """Detect if this is a slice video that needs special handling"""
        slice_url = video_item.get('slice_url')
        if slice_url:
            return True
        
        # Also check if URL contains slice indicators
        url = video_item.get('url', '')
        file_name = video_item.get('file', '')
        
        slice_indicators = ['slice', 'segment', 'chunk', '/media/', '_part']
        return any(indicator in str(url).lower() or indicator in str(file_name).lower() for indicator in slice_indicators)
    
    def get_slice_video_args(self, base_args, video_url):
        """Get specialized VLC arguments for slice videos that are prone to freezing"""
        print(f"DEBUG: Applying slice video optimizations for: {video_url}")
        
        # Slice videos need special network and caching parameters (INCREASED for stability)
        slice_args = [
            '--network-caching=10000',     # 10 second network cache (increased from 3)
            '--file-caching=5000',         # 5 second file cache (increased from 2)
            '--live-caching=3000',         # 3 second live cache (increased from 1)
            '--clock-jitter=0',            # Disable clock jitter for consistent playback
            '--clock-synchro=0',           # Disable clock synchronization
            '--input-repeat=3',            # Allow more input repeats for network issues (increased from 1)
            '--demux-filter=',             # Clear demux filters that might interfere
            '--no-audio-time-stretch',     # Disable audio time stretching
            '--avcodec-skiploopfilter=1',  # Skip loop filter for faster decoding
            '--avcodec-skip-frame=0',      # Don't skip frames
            '--avcodec-skip-idct=0',       # Don't skip IDCT
            '--sout-keep',                 # Keep stream output alive
            '--reset-plugins-cache',       # Reset VLC plugin cache
            '--no-snapshot-preview',       # Disable snapshots that can cause hangs
            '--no-osd',                    # Disable OSD that can interfere
            '--quiet-synchro',             # Quiet synchronization messages
            '--no-keyboard-events',        # CRITICAL: Don't capture keyboard
            '--no-mouse-events',           # CRITICAL: Don't capture mouse
            '--no-interact',               # Disable interactive interface
            '--no-stats',                  # Disable statistics
            '--intf', 'dummy',             # Ensure dummy interface
            '--network-timeout=30',        # 30 second network timeout
            '--http-reconnect',            # Enable HTTP reconnection
            '--http-continuous',           # Enable HTTP continuous streaming
        ]
        
        # Combine base args with slice-specific optimizations
        combined_args = base_args + slice_args + [video_url]
        
        print(f"DEBUG: Slice video VLC args: {' '.join(combined_args[:10])}... ({len(combined_args)} total args)")
        return combined_args
    
    def handle_slice_video_failure(self, video_item, error_msg=""):
        """Handle slice video playback failures with fallback strategies"""
        print(f"DEBUG: ===== SLICE VIDEO FAILURE RECOVERY =====")
        print(f"DEBUG: Error: {error_msg}")
        
        slice_url = video_item.get('slice_url')
        video_file = video_item.get('file')
        
        # Strategy 1: Try to use the original video file instead of slice
        if video_file and slice_url:
            print(f"DEBUG: Attempting fallback to original file: {video_file}")
            fallback_item = video_item.copy()
            fallback_item.pop('slice_url', None)  # Remove slice_url to force original
            
            try:
                self.emergency_stop()  # Clean up any stuck processes
                time.sleep(1)
                self.start_optimized_content(fallback_item)
                return True
            except Exception as e:
                print(f"DEBUG: Fallback to original file failed: {e}")
        
        # Strategy 2: Try to refresh the slice URL from server
        if self.code and video_item.get('id'):
            print("DEBUG: Attempting to refresh slice URL from server")
            try:
                # Get fresh playlist data
                headers = {'X-User-Code': self.code, 'User-Agent': 'EA-TV-Pi-Client/1.0'}
                response = requests.get(f'{self.server_url}/api/playlist/{self.store_id}/{self.screen_id}', 
                                      headers=headers, timeout=10)
                
                if response.status_code == 200:
                    fresh_data = response.json()
                    fresh_items = fresh_data.get('playlist', {}).get('items', [])
                    
                    # Find matching item with fresh slice_url
                    for fresh_item in fresh_items:
                        if fresh_item.get('id') == video_item.get('id'):
                            print("DEBUG: Found fresh slice URL, attempting playback")
                            self.emergency_stop()
                            time.sleep(1)
                            self.start_optimized_content(fresh_item)
                            return True
            except Exception as e:
                print(f"DEBUG: Fresh slice URL fetch failed: {e}")
        
        # Strategy 3: Skip to next scheduled item
        print("DEBUG: All slice recovery strategies failed - forcing next item")
        self.emergency_stop()
        time.sleep(2)
        self.auto_play_next_scheduled()
        return False
    
    def monitor_slice_video_health(self):
        """Monitor slice video health and detect early failure signs"""
        if not self.vlc_process or self.vlc_process.poll() is not None:
            return
            
        current_item = getattr(self, 'current_scheduled_item', None)
        if not current_item or not self.is_slice_video(current_item):
            return
            
        # Check if slice video has been playing too long without activity
        if self.vlc_start_time and (time.time() - self.vlc_start_time) > 60:  # 1 minute
            print("DEBUG: Slice video playing too long - checking health")
            
            try:
                # Try to send a simple command to VLC to test responsiveness
                # If this hangs, VLC is likely frozen
                import signal
                
                def timeout_handler(signum, frame):
                    raise TimeoutError("VLC health check timeout")
                
                signal.signal(signal.SIGALRM, timeout_handler)
                signal.alarm(3)  # 3 second timeout
                
                # Test VLC responsiveness
                vlc_responsive = self.vlc_process.poll() is None
                
                signal.alarm(0)  # Cancel alarm
                
                if not vlc_responsive:
                    print("DEBUG: Slice video VLC appears frozen - initiating recovery")
                    self.handle_slice_video_failure(current_item, "VLC frozen/unresponsive")
                    
            except TimeoutError:
                print("DEBUG: Slice video health check timed out - VLC likely frozen")
                self.handle_slice_video_failure(current_item, "Health check timeout")
            except Exception as e:
                print(f"DEBUG: Slice video health check error: {e}")
        
        # Schedule next health check
        self.root.after(30000, self.monitor_slice_video_health)  # Check every 30 seconds
    ...
    def start_crossfade_transition(self, next_item):
        ...
        try:
            ...
            # Check if this is a slice video and apply specialized handling
            if self.is_slice_video(next_item):
                print("DEBUG: Crossfade target is slice video - applying specialized args")
                # For slice videos in crossfade, we need to be extra careful with larger buffers
                slice_base_args = base_args[2:]  # Remove 'vlc' and '--intf' 
                specialized_args = [
                    '--network-caching=10000',     # 10 second cache for slice videos
                    '--file-caching=5000',         # 5 second file cache
                    '--live-caching=3000',         # 3 second live cache
                    '--video-filter', 'fade',
                    '--fade-in', '1',
                    '--reset-plugins-cache',
                    '--no-snapshot-preview'
                ]
                vlc_args = ['vlc', '--intf', 'dummy'] + specialized_args + slice_base_args + [video_url]
            else:
                # Regular crossfade args
                pass
            ...
            try:
                ...
                print(f"DEBUG: Crossfade VLC startup failed: {e}")
                if self.is_slice_video(next_item):
                    print("DEBUG: Slice video crossfade failed - attempting recovery")
                    self.handle_slice_video_failure(next_item, f"Crossfade startup error: {e}")
                    return
                else:
                    raise
            ...
            # Track new VLC process and activity for freeze detection
            old_vlc_process = self.vlc_process
            ...
            if old_vlc_process and old_vlc_process.poll() is None:
                ...
                self.root.after(1000, lambda: self.fade_out_previous_content(old_vlc_process))
