#!/usr/bin/env python3
"""
Pizza Hut TV - Pi GUI Client (Direct Video URL Version)
Uses direct video URLs instead of playlist endpoint to avoid 403 errors
"""
...
class PizzaHutTVApp:
    ...
    def show_current_step(self):
        ...
        if self.current_step == 'link_code':
            ...
            if screen_key in self.screen_data:
                screen_info = self.screen_data[screen_key]
                video_file = screen_info.get('file', 'No video file')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'screens' in data and data.get('success'):
                    ...
                    self.screen_data = data['screens']  # Store screen data for later use
    ...
    def start_playback(self):
        # Get the actual video file from screen data
        screen_key = f"{self.store_code}_screen{self.screen_id}"
        ...
        if screen_key in self.screen_data:
            ...
            if video_file:
                ...
                video_url = f'{self.server_url}/{video_file}'
                messagebox.showinfo('Starting Playback', f'Playing video:\n{video_file}\n\nURL: {video_url}')
                
                # Start VLC with direct video URL
                try:
                ...
                messagebox.showerror('No Video', f'No video file found for screen {self.screen_id}')
        else:
            messagebox.showerror('No Screen Data', f'No data found for store {self.store_code}, screen {self.screen_id}')
