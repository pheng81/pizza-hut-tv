...
# --- Sync Groups Introspection Endpoint ---
@app.route('/api/sync_groups', methods=['GET'])
def api_sync_groups():
    """Return current sync groups with their members, inferred counts, and timing.
    Response JSON structure:
      { success: true, groups: { group_id: { mode, count, start_epoch, filename, members:[{screen_id, order, role, virtual}], issues:[] } } }
    """
    try:
        header_code = request.headers.get('X-User-Code') or request.args.get('user_code')
        ukey = _resolve_user_key_by_code(header_code)
        if ukey:
            cfg = ensure_playlists_structure(load_store_config_for_user_safe_key(ukey))
        else:
            cfg = load_store_config()
        groups = {}
        raw_groups = (cfg.get('sync_groups') or {}).copy()
        # Build reverse index from playlist items if needed
        for gid, meta in raw_groups.items():
            g = {
                'mode': meta.get('mode') or 'split-h',
                'count': int(meta.get('count') or len(meta.get('members') or []) or 1),
                'start_epoch': int(meta.get('start_epoch') or meta.get('start') or meta.get('created_at') or 0),
                'filename': meta.get('filename'),
                'members': [],
                'issues': []
            }
            # Collect declared members
            for mem in meta.get('members', []):
                try:
                    g['members'].append({
                        'screen_id': mem.get('screen_id'),
                        'order': mem.get('order'),
                        'role': mem.get('role') or ('master' if mem.get('order',0)==0 else 'follower'),
                        'virtual': False
                    })
                except Exception:
                    g['issues'].append('member_parse_failed')
            groups[gid] = g
        # Scan actual playlists for discrepancies / inferred items
        screens = cfg.get('screens') or {}
        for store_id, scr_map in screens.items():
            for scr_id, scr_data in scr_map.items():
                for it in scr_data.get('playlist', []):
                    ...
                    if gid not in groups:
                        groups[gid] = {
                            'mode': sref.get('mode') or 'split-h',
                            'count': int(sref.get('count') or 1),
                            'start_epoch': int(sref.get('start_epoch') or 0),
                            'filename': it.get('file'),
                            'members': [],
                            'issues': ['group_missing_in_config']
                        }
                    # Check if screen/member already included
                    exists = any(m.get('screen_id') in {scr_id, f"{store_id}_{scr_id}"} for m in groups[gid]['members'])
                    if not exists:
                        groups[gid]['members'].append({
                            'screen_id': scr_id,
                            'order': sref.get('order'),
                            'role': sref.get('role') or ('master' if (sref.get('order') or 0)==0 else 'follower'),
                            'virtual': sref.get('virtual') or False
                        })
                    # Update count/mode if config missing/zero
                    try:
                        if groups[gid]['count'] < 1 and sref.get('count'):
                            groups[gid]['count'] = int(sref.get('count'))
                        if not groups[gid].get('mode') and sref.get('mode'):
                            groups[gid]['mode'] = sref.get('mode')
                    except Exception:
                        pass
        # Sort members by order for readability
        for g in groups.values():
            try:
                g['members'].sort(key=lambda m: (m.get('order') if m.get('order') is not None else 9999))
            except Exception:
                pass
...
# --- Sync Slice Manifest Endpoint ---
@app.route('/api/sync_slice_manifest', methods=['GET'])
def api_sync_slice_manifest():
    """Provide a lightweight manifest describing slice URLs for every member of a sync group.
    Query params:
      group: required sync group id
      absolute=1 : include absolute slice_url (default 1)
      refresh=1  : append refresh=1 to slice urls to force re-encode/regenerate
      probe=1    : attempt to read sidecar metadata (audio presence) and filesize
    Response:
      { success: true, group: {id, mode, count, filename}, slices:[{screen_id, order, slice_url, cached, has_audio, size_bytes}] }
    Notes:
      This avoids each Android TV performing its own order discovery; the controller can fetch once then
      push correct URL to each screen.
    """
    gid = request.args.get('group') or request.args.get('id')
    if not gid:
        return jsonify({'success': False, 'error': 'group param required'}), 400
    try:
        header_code = request.headers.get('X-User-Code') or request.args.get('user_code')
        ukey = _resolve_user_key_by_code(header_code)
        if ukey:
        ...
        mode = (group.get('mode') or 'split-h').lower()
        filename = group.get('filename')
        members = group.get('members') or []
        count = int(group.get('count') or len(members) or 1)
        refresh_flag = '1' if (request.args.get('refresh') == '1') else None
        want_probe = request.args.get('probe') == '1'
        absolute = request.args.get('absolute', '1') != '0'
        slices = []
        for m in members:
            try:
                s_id = m.get('screen_id')
                order = int(m.get('order')) if m.get('order') is not None else 0
                if not filename:
                    slices.append({'screen_id': s_id, 'order': order, 'error': 'no filename on group'})
                    continue
                # Build slice URL
                base = url_for('slice_video', video_path=filename, _external=absolute)
                qs = f"slice_mode={mode}&slice_count={count}&slice_order={order}"
                if refresh_flag:
                    qs += '&refresh=1'
                slice_url = base + '?' + qs
                entry = {'screen_id': s_id, 'order': order, 'slice_url': slice_url}
                # Sidecar / cache probe
                if want_probe:
                    try:
                        slice_path = os.path.join('static', 'cache', 'slices', f"{filename}.meta.json")
                        # meta is for the full file; each slice shares audio characteristics
                        meta = None
                        if os.path.exists(slice_path):
                            with open(slice_path, 'r', encoding='utf-8', errors='ignore') as fh:
                                meta = json.load(fh)
                        if meta:
                            entry['has_audio'] = bool(meta.get('has_audio'))
                        # Attempt to locate cached slice file name pattern: <video>.slice.<mode>.<count>.<order>.mp4
                        cache_dir = os.path.join('static', 'cache', 'slices')
                        # Iterate predictable filename first
                        pattern_name = f"{filename}.slice.{mode}.{count}.{order}.mp4"
                        cache_file = os.path.join(cache_dir, pattern_name)
                        if os.path.exists(cache_file):
                            entry['cached'] = True
                            try:
                                entry['size_bytes'] = os.path.getsize(cache_file)
                            except Exception:
                                pass
                        else:
                            entry['cached'] = False
                    except Exception as e_probe:
                        entry['probe_error'] = str(e_probe)
                slices.append(entry)
            except Exception as e_m:
                slices.append({'screen_id': m.get('screen_id'), 'error': str(e_m)})
        return jsonify({'success': True, 'group': {'id': gid, 'mode': mode, 'count': count, 'filename': filename}, 'slices': slices})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

# --- Rebuild Sync Groups From Playlist Items (Recovery Utility) ---
@app.route('/api/rebuild_sync_groups', methods=['POST','GET'])
def api_rebuild_sync_groups():
    """Scan all playlists, discover items with sync_ref.group and rebuild any missing
    top-level sync_groups entries. Useful if config lost its sync_groups but each screen
    item still carries sync_ref metadata, resulting in empty /api/sync_groups.

    Query / Body params:
      dry=1   -> do not persist, just report what would be added
      force=1 -> overwrite existing groups that have zero members or no filename

    Returns: { success, discovered:N, rebuilt:M, skipped:K, groups:[ {id, members, filename, mode, count, action} ] }
    """
    ...
    try:
        ...
        if ukey:
            ...
            cfg = ensure_playlists_structure(load_store_config())
        existing = cfg.get('sync_groups') or {}
        if not isinstance(existing, dict):
            existing = {}
        groups_index = {}
        # Discover
        screens_map_all = (cfg.get('screens') or {})
        for store_id, scrs in screens_map_all.items():
            for screen_id, sdata in (scrs or {}).items():
                for item in (sdata.get('playlist') or []):
                    if not isinstance(item, dict):
                        continue
                    sref = item.get('sync_ref') if isinstance(item.get('sync_ref'), dict) else None
                    if not sref:
                        continue
                    gid = sref.get('group')
                    if not gid:
                    ...
                    if cnt <= 1:
                        continue
                    mode = (sref.get('mode') or 'split-h').lower()
                    order = 0
                    try:
                    ...
                    entry = groups_index.setdefault(gid, {
                        'members': [],
                        'filename': filename,
                        'mode': mode,
                        'count_hint': cnt,
                        'orders': set(),
                        'store_id': store_id,
                        'start_epoch': sref.get('start_epoch'),
                    })
                    # Prefer consistent filename; if mismatch across members note it
                    if filename and entry.get('filename') != filename:
                        entry.setdefault('issues', []).append('filename_mismatch')
                    entry['members'].append({'screen_id': screen_id, 'item_id': item.get('id'), 'role': sref.get('role') or ('master' if order == 0 else 'follower'), 'order': order})
                    entry['orders'].add(order)
                    if cnt > entry.get('count_hint', 1):
                        entry['count_hint'] = cnt
        actions = []
        rebuilt = 0
        skipped = 0
        for gid, info in groups_index.items():
            members = info['members']
            if not members:
                continue
            filename = info.get('filename')
            mode = info.get('mode') or 'split-h'
            count_val = max(info.get('count_hint') or len(members), len(members))
            start_epoch = info.get('start_epoch') or int(time.time()) + 5
            existing_entry = existing.get(gid)
            if existing_entry and not force:
                actions.append({'id': gid, 'action': 'skip-exists', 'members': len(members)})
                skipped += 1
                continue
            new_entry = {
                'store_id': info.get('store_id'),
                'base': members[0]['screen_id'] if members else None,
                'count': count_val,
                'filename': filename,
                'members': members,
            ...
            actions.append({'id': gid, 'action': 'rebuild', 'members': len(members), 'count': count_val})
            existing[gid] = new_entry
            rebuilt += 1
        result = {'success': True, 'discovered': len(groups_index), 'rebuilt': rebuilt, 'skipped': skipped, 'groups': actions, 'dry': dry}
        if not dry and rebuilt:
            cfg['sync_groups'] = existing
            try:
                save_store_config(cfg)
            except Exception as e_save:
                result['save_error'] = str(e_save)
        return jsonify(result)
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

# --- Ensure single sync group exists based on sync_ref items (idempotent) ---
@app.route('/api/ensure_sync_group', methods=['POST','GET'])
def api_ensure_sync_group():
    """Create or repair a specific sync group entry when playlist items have sync_ref metadata but
    the top-level sync_groups entry is missing. Uses user_code if provided.
    Params: group=<gid>
      Optional: mode=split-h (override), count=<n> (override), dry=1
    Returns: { success, action: created|updated|exists|not_found, group_id, members, count }
    """
    gid = request.args.get('group') or request.args.get('id')
    if not gid:
        return jsonify({'success': False, 'error': 'group param required'}), 400
    ...
    try:
        ...
        if ukey:
            ...
            cfg = ensure_playlists_structure(load_store_config())
        existing = (cfg.get('sync_groups') or {})
        if gid in existing and not dry:
            return jsonify({'success': True, 'action': 'exists', 'group_id': gid, 'count': existing[gid].get('count'), 'members': len(existing[gid].get('members') or [])})
        # Scan for members with this group id
        members = []
        filename = None
        mode_override = (request.args.get('mode') or '').strip().lower() or None
        count_override = request.args.get('count')
        screens_map_all = (cfg.get('screens') or {})
        for store_id, scrs in screens_map_all.items():
            for screen_id, sdata in (scrs or {}).items():
                for item in (sdata.get('playlist') or []):
                    if not isinstance(item, dict):
                        continue
                    sref = item.get('sync_ref') if isinstance(item.get('sync_ref'), dict) else None
                    if not sref or sref.get('group') != gid:
                        continue
                    order = 0
                    try:
                        order = int(sref.get('order') or 0)
                        ...
                        pass
                    role = sref.get('role') or ('master' if order == 0 else 'follower')
                    f = item.get('file')
                    if f and not filename:
                        filename = f
                    members.append({'screen_id': screen_id, 'item_id': item.get('id'), 'role': role, 'order': order})
        if not members:
            return jsonify({'success': False, 'error': 'no playlist items with that group id found'}), 404
        # Determine count
        max_order = max((m.get('order') or 0) for m in members)
        derived_count = max_order + 1
        try:
            if count_override is not None:
                derived_count = int(count_override)
        except Exception:
            pass
        # Determine mode
        mode = mode_override or 'split-h'
        # Build group entry
        now = int(time.time())
        entry = {
            'store_id': members[0]['screen_id'].split('_',1)[0] if '_' in members[0]['screen_id'] else None,
            'base': members[0]['screen_id'],
            'count': derived_count,
            'filename': filename,
            'members': members,
            'created_at': now,
        ...
        if dry:
            return jsonify({'success': True, 'action': 'dry-preview', 'group_id': gid, 'count': derived_count, 'members': len(members)})
        cfg.setdefault('sync_groups', {})[gid] = entry
        # Persist
        if ukey:
            save_store_config_for_user_safe_key(ukey, cfg)
        else:
            save_store_config(cfg)
        return jsonify({'success': True, 'action': 'created', 'group_id': gid, 'count': derived_count, 'members': len(members)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

# --- Slice Health Check Endpoint ---
@app.route('/api/check_slice_health', methods=['GET'])
def api_check_slice_health():
    """Inspect health of a video slice (or all slices for a file) and optionally regenerate.
    Query params:
      file=users/.../video.mp4 (required)
      slice_mode=split-h|split-v (default split-h)
      slice_count=N (default 3)
      order=K or 'all' (default all)
      refresh=1 to force regeneration
    Returns: { success, file, slices:[{order, exists, size_bytes, audio, path, regenerated, error?}], notes:[] }
    """
    try:
        # Accept legacy/alias params: video_path, count
        vfile = (request.args.get('file') or request.args.get('video_path') or '').strip()
        if not vfile:
            return jsonify({'success': False, 'error': 'file param required (use file= or video_path=)'}), 400
        # Accept alias 'mode' for slice_mode for user convenience
        mode = (request.args.get('slice_mode') or request.args.get('mode') or 'split-h').lower()
        # Allow 'count' alias for slice_count
        raw_count = request.args.get('slice_count') or request.args.get('count') or 3
        try:
            count = int(raw_count)
        except Exception:
            count = 3
        order_param = request.args.get('order') or 'all'
        force = str(request.args.get('refresh') or '').lower() in ('1','true','yes')
        if count < 1: count = 1
        orders = list(range(count)) if order_param == 'all' else []
        if not orders:
            try:
                single = int(order_param)
                if 0 <= single < count:
                    orders = [single]
            except Exception:
                return jsonify({'success': False, 'error': 'invalid order'}), 400
        else:
            # Sanity: if user requested all but we somehow didn't expand fully
            if len(orders) != count:
                print(f"WARNING: slice_health mismatch orders({orders}) != count({count})")
        results = []
        notes = []
        print(f"DEBUG: slice_health start file={vfile} mode={mode} count={count} orders={orders} force={force}")
        for ordx in orders:
            cache_key = f"{SLICE_ENCODER_VERSION}__{vfile.replace('/', '_').replace('\\', '_')}_slice_{mode}_{count}_{ordx}"
            slice_path = os.path.join(SLICE_CACHE_FOLDER, f"{cache_key}.mp4")
            existed_before = os.path.exists(slice_path)
            regenerated = False
            audio_state = 'unknown'
            error = None
            if (not existed_before) or force:
                ...
                if not FFMPEG_AVAILABLE:
                    ...
                    src_path = os.path.join(app.config['UPLOAD_FOLDER'], vfile)
                    if not os.path.exists(src_path) and r2_enabled():
                        try:
                            r2_download_to_local(vfile)
                        except Exception as e_r2:
                            error = f'restore_failed:{e_r2}'
                    if os.path.exists(src_path) and not error:
                        ok = generate_video_slice_ffmpeg(src_path, slice_path, mode, count, ordx)
                        regenerated = ok
                        if not ok:
                            error = 'generation_failed'
            if os.path.exists(slice_path):
                try:
                    size_b = os.path.getsize(slice_path)
                except Exception:
                    size_b = 0
                # Probe audio if ffprobe available
                if FFMPEG_AVAILABLE:
                    try:
                        probe_cmd = [FFPROBE_PATH or (FFMPEG_PATH.replace('ffmpeg','ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe'), '-v','error','-select_streams','a','-show_entries','stream=codec_type','-of','csv=p=0', slice_path]
                        pr = subprocess.run(probe_cmd, capture_output=True, text=True, timeout=5)
                        if pr.returncode == 0:
                            outl = [ln.strip() for ln in pr.stdout.splitlines() if ln.strip()]
                            audio_state = 'present' if any('audio' in ln for ln in outl) else 'missing'
                        else:
                            audio_state = 'probe_error'
                    except Exception as e_a:
                        audio_state = f'probe_exc:{e_a}'
                results.append({'order': ordx, 'exists': True, 'size_bytes': size_b, 'audio': audio_state, 'path': slice_path, 'regenerated': regenerated, 'error': error})
            else:
                results.append({'order': ordx, 'exists': False, 'size_bytes': 0, 'audio': 'n/a', 'path': slice_path, 'regenerated': regenerated, 'error': error or 'missing'})
        resp_payload = {'success': True, 'file': vfile, 'mode': mode, 'count': count, 'requested_order': order_param, 'resolved_orders': orders, 'ffprobe_available': FFMPEG_AVAILABLE, 'slices': results, 'notes': notes}
        # Add quick summary stats
        try:
            present = sum(1 for s in results if s['exists'])
            audio_missing = [s['order'] for s in results if s['exists'] and s.get('audio') not in ('present','true')]
            resp_payload['summary'] = {'slices_present': present, 'audio_missing_orders': audio_missing}
        except Exception:
            pass
        print(f"DEBUG: slice_health done file={vfile} orders={orders} present={[s['order'] for s in results if s['exists']]} audio={[s.get('audio') for s in results]}")
...
def stream_media(filename):
    ...
    if request.method == 'HEAD':
        ...
        # Android TV ExoPlayer compatibility headers
        resp.headers.add('Access-Control-Allow-Origin', '*')
        resp.headers.add('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS')
        resp.headers.add('Access-Control-Allow-Headers', 'Range, Content-Type, Content-Length')
        resp.headers.add('Access-Control-Expose-Headers', 'Content-Range, Content-Length, Accept-Ranges')
        try:
            resp.set_etag(etag)
        except Exception:
...
def save_store_config_for_user_safe_key(safe_key: str, config):
    ...
    try:
        path = _config_path_for_user_safe_key(safe_key)
        _atomic_write_with_retry(path, config, label=f'user:{safe_key}')
    except Exception as e:
...
def save_store_config(config):
    ...
    try:
        ...
        _atomic_write_with_retry(cfg_path, config, label='global')
    except Exception as e:
        print(f"Error saving configuration: {e}")
        raise

def _atomic_write_with_retry(path: str, data_obj, attempts: int = 5, base_delay: float = 0.05, label: str = ''):
    """Atomically write JSON to path with retry/backoff for transient Windows PermissionError (WinError 5).
    Writes to <path>.tmp then os.replace().
    Parameters:
      attempts   : total attempts (>=1)
      base_delay : initial sleep seconds (exponential backoff applied)
    ...
    for i in range(1, attempts + 1):
        ...
        try:
            ...
            os.replace(tmp, path)
            print(f"CONFIG: atomic save ok ({label}) -> {path} (attempt {i})")
            return
        except PermissionError as pe:
            last_err = pe
            # On Windows, another process (virus scanner, indexing) may lock momentarily
            wait = base_delay * (2 ** (i - 1))
            print(f"WARNING: PermissionError saving config ({label}) attempt {i}/{attempts}: {pe}; retrying in {wait:.2f}s")
            try:
                time.sleep(wait)
            except Exception:
                pass
            ...
            last_err = e
            print(f"ERROR: atomic write failed ({label}) attempt {i}/{attempts}: {e}")
            # For non-permission errors, still retry a couple of times in case of transient IO
            wait = base_delay * (2 ** (i - 1))
            try:
                time.sleep(wait)
            except Exception:
                pass
    # Exhausted attempts
    raise last_err if last_err else RuntimeError('unknown error in _atomic_write_with_retry')
...
if FFMPEG_AVAILABLE:
    # Derive ffprobe path and verify
    cand = FFMPEG_PATH.replace('ffmpeg','ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe'
    try:
        pr = subprocess.run([cand,'-version'], capture_output=True, text=True, timeout=5)
        if pr.returncode == 0:
            FFPROBE_PATH = cand
            print(f"DEBUG: ffprobe detected: {cand}")
        else:
            print(f"WARNING: ffprobe invocation failed: {pr.stderr[:120]}")
    except Exception as _e_fp:
        print(f"WARNING: ffprobe not available ({_e_fp}) - will use fallback dimensions")
if not FFMPEG_AVAILABLE:
...
def get_playlist(store_id, screen_id):
    ...
    out = []
    # Optional: inherit first sync_ref across subsequent video items on same screen.
    # Use query ?inherit_sync_ref=1 or env ALWAYS_INHERIT_SYNC_REF=1 to enable.
    inherit_sync_ref = False
    try:
        if os.environ.get('ALWAYS_INHERIT_SYNC_REF') in ('1','true','yes','on'):
            inherit_sync_ref = True
        if request.args.get('inherit_sync_ref') in ('1','true','yes','on'):
            inherit_sync_ref = True
    except Exception:
        pass
    inherited_sref_template = None  # cached first group's template (without order) for cloning
    for item in pl:
        try:
            it = dict(item)
            ...
            try:
                ...
                if not sref and inherit_sync_ref and inherited_sref_template and (it.get('media_type') == 'video' or classify_media(it.get('file') or '') == 'video'):
                    # Clone template; order stays same as original unless we purposely leave None
                    cloned = dict(inherited_sref_template)
                    # Keep original order for all followers to maintain consistent slicing region per screen.
                    sref = cloned
                    it['sync_ref'] = dict(cloned)
                    it['sync_ref']['_inherited'] = True
                gid = None
                ...
                if gid:
                    ...
                    try:
                        ...
                        try:
                            ...
                            it['sync_ref']['order'] = 0
                    # Capture template for inheritance AFTER normalization (excluding order so each screen preserves its assigned order)
                    try:
                        if inherit_sync_ref and not inherited_sref_template:
                            templ = dict(it.get('sync_ref') or {})
                            # Remove volatile timing and flags
                            for k in ('start_epoch','_inherited'):
                                templ.pop(k, None)
                            inherited_sref_template = templ
                    except Exception:
                        pass
            except Exception:
                pass
            ...
            try:
                ...
                if isinstance(sref2, dict) and media_type == 'video':
                    ...
                    # Debug overrides to help diagnose follower screens
                    try:
                        if request.args.get('force_slice_order') is not None:
                            forced = int(request.args.get('force_slice_order'))
                            print(f"DEBUG: force_slice_order override -> {forced} (was {sorder})")
                            sorder = forced
                        if request.args.get('force_android') is not None:
                            # Treat request as Android client regardless of UA
                            if request.args.get('force_android') in ('1','true','yes','on','android'):
                                ua_effective_local = 'android-debug-override'
                            else:
                                ua_effective_local = ua_effective
                        else:
                            ua_effective_local = ua_effective
                    except Exception:
                        ua_effective_local = ua_effective
                    if scount > 1:
                        ua = ua_effective_local
                        # Expandable Android/TV detection tokens
                        base_android_tokens = ['android', 'okhttp', 'exoplayer', 'nvidia', 'bravia', 'shield']
                        # Broad smart TV / streaming device tokens
                        broad_tokens = ['smarttv', 'firetv', 'tizen', 'webos', 'leanback', 'chromecast', 'aftmm', 'aftss', 'aftm', 'aftb', 'atv', 'emulator', 'tv']
                        try:
                            extra_env = os.environ.get('ANDROID_UA_EXTRA_TOKENS')
                            if extra_env:
                                for t in extra_env.split(','):
                                    t2 = t.strip().lower()
                                    if t2 and t2 not in broad_tokens and t2 not in base_android_tokens:
                                        broad_tokens.append(t2)
                        except Exception:
                            pass
                        android_tokens = tuple(base_android_tokens + broad_tokens)
                        is_android = any(tok in ua for tok in android_tokens)
                        always_slice = False
                        try:
                            if os.environ.get('ALWAYS_SLICE_SYNC') in ('1','true','yes','on'):
                                always_slice = True
                            if request.args.get('always_slice') in ('1','true','yes','on'):
                                always_slice = True
                        except Exception:
                            pass
                        if always_slice and scount > 1:
                            is_android = True
                        if request.args.get('debug') == '1':
                            it['debug_ua_tokens'] = list(android_tokens)
                            it['debug_ua'] = ua
                            it['debug_is_android'] = is_android
                            it['debug_always_slice'] = always_slice
                        vfile = it.get('file')
                        ...
                        if vfile:
                            ...
                            # TEMPORARY FIX: Disable slicing for Android TV to test original video playback
                            android_ua = 'phtv-android' in ua.lower()
                            if android_ua:
                                # Decide whether to force original or use slice based on env/query
                                force_original = False
                                try:
                                    env_force = os.environ.get('ANDROID_TV_FORCE_ORIGINAL','').lower() in ('1','true','yes','on')
                                    arg_force = request.args.get('android_force_original','').lower() in ('1','true','yes','on')
                                    force_original = env_force or arg_force
                                except Exception:
                                    pass
                                # Build absolute original URL for fallback / alternative
                                try:
                                    orig_file = it.get('file') or vfile
                                    if orig_file:
                                        abs_orig_url = url_for('stream_media', filename=orig_file, _external=True)
                                        it['absolute_original_url'] = abs_orig_url
                                        it['absolute_media_url'] = abs_orig_url
                                except Exception as _e_abs:
                                    print(f"WARNING: Android abs URL build failed: {_e_abs}")
                                if force_original:
                                    is_android = False  # prevent slice assignment logic below
                                    it['url'] = it.get('absolute_media_url') or (base_url if base_url else it.get('url'))
                                    it['android_forced_original'] = True
                                    it['android_play_mode'] = 'original'
                                    print(f"DEBUG: ANDROID TV using ORIGINAL (forced) UA={ua}")
                                else:
                                    # Keep Android slicing behavior; ensure we expose slice_url as final url
                                    is_android = True
                                    it['android_play_mode'] = 'slice'
                                    print(f"DEBUG: ANDROID TV using SLICE mode UA={ua}")
                            if is_android:
                                # For Android (non TV test), prefer server-sliced URL
                                it['url'] = slice_url
                                it['android_adjusted'] = True
                                print(f"DEBUG: Assigned slice URL (Android): {slice_url}")
                            else:
                                # Non-Android (or Android TV test path): keep original base URL but expose slice_url
                                print(f"DEBUG: Exposed slice_url (non-Android-or-TV-test): {slice_url}")
                            # Mark whether slice actually applied to main url for client diagnostics
                            it['slice_applied'] = (it.get('url') == slice_url)
                            # DEBUG: Log final URL for Android TV
                            if android_ua:
                                print(f"DEBUG: ANDROID TV FINAL URL: {it.get('url', 'NO URL SET')}")
                        else:
                            print("DEBUG: Cannot assign slice URL (missing file field)")
                            it['slice_applied'] = False
                    else:
                        it['slice_applied'] = False
                else:
                    # Not a slice scenario
                    if isinstance(it, dict):
                        it['slice_applied'] = False
            except Exception as e_slice:
                print(f"WARNING: Slice URL assignment error: {e_slice}")
            # Simplified image display logic: ALWAYS contain (fit entire image) unless explicit query param overrides.
            try:
                mt_hint = it.get('media_type') or item.get('media_type') or classify_media(it.get('file') or '')
                if mt_hint == 'image':
                    override = (request.args.get('image_fit') or request.args.get('img_fit') or '').strip().lower()
                    # Normalize limited accepted overrides
                    if override in ('cover','full','fullscreen','fill','crop'):
                        fit = 'cover'
                        image_scale = 'center_crop'
                        reason = 'explicit_override'
                    elif override in ('stretch'):
                        fit = 'stretch'
                        image_scale = 'fit_xy'
                        reason = 'explicit_override'
                    else:
                        fit = 'contain'
                        image_scale = 'fit_center'
                        reason = 'default_contain'
                    it['image_fit'] = fit
                    it['image_scale'] = image_scale
                    it['image_fit_reason'] = reason
            except Exception as _img_e:
                print(f"DEBUG: image hint error simplified: {_img_e}")
            ...
            out.append(item)
    # --- Persist inferred sync_ref fields back to configuration (one save per request) ---
    try:
        dirty = False
        # We only persist for real playlist items (exclude virtual synthesized later)
        for idx, original in enumerate(pl):
            if not isinstance(original, dict):
                continue
            oref = original.get('sync_ref') if isinstance(original.get('sync_ref'), dict) else None
            # Find corresponding enriched version
            enriched = None
            try:
                enriched = out[idx]
            except Exception:
                enriched = None
            eref = enriched.get('sync_ref') if isinstance(enriched, dict) else None
            if not (oref or eref):
                continue
            # Only persist if we have a group id and at least one of the critical fields was missing originally
            try:
                gid = (eref or oref).get('group') if isinstance((eref or oref), dict) else None
            except Exception:
                gid = None
            if not gid:
                continue
            # Determine if persistence needed
            need_fields = []
            for f in ('count', 'mode', 'order', 'start_epoch'):
                if isinstance(eref, dict):
                    if not (isinstance(oref, dict) and f in oref and oref.get(f) not in (None, '')) and eref.get(f) not in (None, ''):
                        need_fields.append(f)
            if not need_fields:
                continue
            # Apply back to original structure kept in cfg (cfg -> stores[store_id]['screens'][screen_id]['playlist'])
            try:
                for f in need_fields:
                    # Ensure sync_ref dict exists on original
                    original.setdefault('sync_ref', {})
                    original['sync_ref'][f] = eref.get(f)
                dirty = True
            except Exception:
                pass
        if dirty:
            # Save only once
            try:
                # Persist using general save function (handles backups)
                save_store_config(cfg)
            except Exception as e_save:
                print(f"WARNING: Failed to persist inferred sync_ref fields: {e_save}")
    except Exception:
        pass
    ...
    try:
        ...
        # TEMPORARY: Disable slicing for Android TV to test original videos
        android_tv = 'phtv-android' in ua.lower()
        if android_tv:
            # Disable slicing AND remember flag
            is_android = False
            print(f"DEBUG: TEMP FIX #2 - Disabled slicing for Android TV: {ua}")
        # Optionally expose tokens once for debugging (attach to first element if slice scenario)
        if request.args.get('debug') == '1' and out:
            try:
                if isinstance(out[0], dict):
                    out[0]['debug_ua_tokens'] = list(android_tokens)
                    out[0]['debug_ua'] = ua
                    out[0]['debug_is_android'] = is_android
                    out[0]['debug_always_slice'] = always_slice
            except Exception:
                pass
        for i, it in enumerate(out):
            try:
                ...
                if android_tv:
                    # Ensure absolute original URL present for fallback
                    try:
                        if 'absolute_original_url' not in it:
                            orig_file = it.get('file') or vfile
                            if orig_file:
                                absu = url_for('stream_media', filename=orig_file, _external=True)
                                it['absolute_original_url'] = absu
                                it['absolute_media_url'] = absu
                        # Determine desired mode from flags already set
                        play_mode = it.get('android_play_mode') or ('original' if it.get('android_forced_original') else 'slice')
                        if play_mode == 'original':
                            if it.get('absolute_original_url') and cur_url != it['absolute_original_url']:
                                it['url'] = it['absolute_original_url']
                                it['android_adjusted_post'] = True
                        else:  # slice mode
                            if '/slice-video/' not in cur_url:
                                # Promote slice_url to main url if available
                                if it.get('slice_url'):
                                    it['url'] = it['slice_url']
                                    it['android_adjusted_post'] = True
                        it['android_final_mode'] = play_mode
                    except Exception as _e_and_abs:
                        print(f"WARNING: Android TV post abs url build failed: {_e_and_abs}")
                    print(f"DEBUG: Post-slice ANDROID TV path - final url='{it.get('url')}' mode={it.get('android_final_mode')} abs_orig='{it.get('absolute_original_url')}' slice_url='{it.get('slice_url')}'")
                elif is_android and '/slice-video/' not in cur_url:
                    it['url'] = slice_url
                    print(f"DEBUG: Post-assigned slice URL for item[{i}] (Android): {slice_url}")
                else:
                    print(f"DEBUG: Post-slice NOT assigned - is_android={is_android}, android_tv={android_tv}, cur_url='{cur_url}'")
...
def serve_video_file_with_range_support(file_path, is_cached='false'):
    ...
    if range_header:
        try:
            ...
            # Sidecar metadata: expose audio presence if available
            try:
                if is_cached == 'true' and 'X-Slice-Audio' not in resp.headers:
                    meta_path = file_path + '.meta.json'
                    if os.path.exists(meta_path):
                        import json as _json2
                        with open(meta_path, 'r', encoding='utf-8') as mf:
                            mdata = _json2.load(mf)
                        ha = mdata.get('has_audio')
                        if ha is not None:
                            resp.headers['X-Slice-Audio'] = 'true' if ha else 'false'
            except Exception as _side_e:
                print(f"DEBUG: sidecar header inject failed: {_side_e}")
...
@app.route('/api/debug_pair_codes')
def debug_pair_codes():
    """Debug endpoint to list all active pairing codes and their associated users."""
    try:
        db = get_db()
        rows = db.execute('SELECT username, link_code FROM users WHERE link_code IS NOT NULL ORDER BY username').fetchall()
        codes = [{'username': row['username'], 'code': row['link_code']} for row in rows]
        return {'success': True, 'active_codes': codes, 'count': len(codes)}
    except Exception as e:
        return {'success': False, 'error': str(e)}, 500

@app.route('/api/fix_pair_code/<code>')
def fix_pair_code(code):
    """Fix pairing code by updating username to use _at_ format."""
    try:
        db = get_db()
        # Get current username for this code
        row = db.execute('SELECT username FROM users WHERE link_code = ?', (code,)).fetchone()
        if not row:
            return {'success': False, 'error': 'Code not found'}, 404
        
        old_username = row['username']
        # Convert to _at_ format
        new_username = old_username.replace('@', '_at_')
        
        # Update the database
        db.execute('UPDATE users SET username = ? WHERE link_code = ?', (new_username, code))
        db.commit()
        
        return {'success': True, 'old_username': old_username, 'new_username': new_username, 'code': code}
    except Exception as e:
        return {'success': False, 'error': str(e)}, 500

# Video slicing endpoint for Android TV compatibility
SLICE_ENCODER_VERSION = "v3-faststart-20250917"
...
def slice_video(video_path):
    ...
    try:
        ...
        sync_group = request.args.get('sync_group') or ''

        print(f"DEBUG: slice_video request - path={video_path}, mode={slice_mode}, count={slice_count}, order={slice_order}, sync_group={sync_group}")
        # Optional: force refresh to regenerate slice with latest encoder settings
        force_refresh = str(request.args.get('refresh') or '').strip().lower() in ('1', 'true', 'yes')

        # Reject obviously non-video extensions to reduce noisy 302/404 chains (.png/.jpg requested as slice)
        lower_path = video_path.lower()
        if lower_path.endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp')):
            return jsonify({
                'success': False,
                'error': 'not a video file for slicing',
                'file': video_path,
                'hint': 'Remove slice_mode/count params for images or use the original media URL. This endpoint expects a .mp4 or video extension.'
            }), 400

        # Lightweight opportunistic cache pruning: keep at most 200 slice files; remove oldest beyond that
        try:
            max_slice_files = 200
            if os.path.isdir(SLICE_CACHE_FOLDER):
                files = [os.path.join(SLICE_CACHE_FOLDER, f) for f in os.listdir(SLICE_CACHE_FOLDER) if f.endswith('.mp4')]
                if len(files) > max_slice_files:
                    files.sort(key=lambda p: os.path.getmtime(p))
                    to_delete = files[0: len(files) - max_slice_files]
                    for fp in to_delete:
                        try:
                            os.remove(fp)
                        except Exception:
        ...
        if success and os.path.exists(cached_slice_path):
            ...
            resp = serve_video_file_with_range_support(cached_slice_path, 'false')
            if sync_group:
                try:
                    resp.headers['X-Sync-Group'] = sync_group
                except Exception:
                    pass
            try:
                resp.headers['X-Sync-Order'] = str(slice_order)
            except Exception:
                pass
            # Detect audio presence quickly (size heuristic or ffprobe if available)
            try:
                has_audio = 'unknown'
                if FFMPEG_AVAILABLE:
                    probe_cmd = [FFPROBE_PATH or (FFMPEG_PATH.replace('ffmpeg','ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe'), '-v','error','-select_streams','a','-show_entries','stream=codec_type','-of','csv=p=0', cached_slice_path]
                    pr = subprocess.run(probe_cmd, capture_output=True, text=True, timeout=5)
                    if pr.returncode == 0:
                        outl = [ln.strip() for ln in pr.stdout.splitlines() if ln.strip()]
                        has_audio = 'true' if any('audio' in ln for ln in outl) else 'false'
                resp.headers['X-Slice-Audio'] = has_audio
            except Exception:
                pass
            return resp
        else:
...
@app.route('/api/preview_slice_mosaic')
def preview_slice_mosaic():
    """Return a horizontal mosaic (JPEG) of the first video frame of each slice for a given ultra-wide video.
    Query params:
      video_path : relative path to source video (same as used for /slice-video)
      slice_mode : split-h (default) or split-v
      slice_count: number of slices
      t          : timestamp (seconds) to grab frame (default 0.5)
      regen      : if '1' force regenerate any missing slice before grabbing frame
      scale_h    : optional target slice height for mosaic (default 240)
    """
    try:
        import subprocess, io, os, json as _json
        from PIL import Image  # type: ignore
    except Exception as e:
        return jsonify({'error': 'Pillow or system dependency missing', 'detail': str(e)}), 500
    ...
    try:
        ...
        slice_count = 1
    ts = request.args.get('t', '0.5')
    try:
        ts_float = max(0.0, float(ts))
    except Exception:
        ts_float = 0.5
    force_regen = (request.args.get('regen','0').lower() in ('1','true','yes'))
    try:
        target_h = int(request.args.get('scale_h', 240))
    except Exception:
        target_h = 240

    # Basic validation
    if slice_count < 1 or slice_count > 32:
        return jsonify({'error': 'slice_count out of range (1..32)'}), 400

    # Ensure cache folder exists
    os.makedirs(SLICE_CACHE_FOLDER, exist_ok=True)

    # For each slice, ensure slice video exists (generate if needed) then extract a frame to an Image
    images = []
    base_errors = []
    for ordx in range(slice_count):
        cache_key = f"{SLICE_ENCODER_VERSION}__{video_path.replace('/', '_').replace('\\', '_')}_slice_{slice_mode}_{slice_count}_{ordx}"
        slice_path = os.path.join(SLICE_CACHE_FOLDER, f"{cache_key}.mp4")
        # Regenerate if asked or missing
        if (force_regen or not os.path.exists(slice_path)) and FFMPEG_AVAILABLE:
            try:
                original_video_path = os.path.join(app.config['UPLOAD_FOLDER'], video_path)
                if not os.path.exists(original_video_path):
                    base_errors.append(f'source-missing:{video_path}')
                else:
                    generate_video_slice_ffmpeg(original_video_path, slice_path, slice_mode, slice_count, ordx)
            except Exception as ge:
                base_errors.append(f'gen-fail:{ordx}:{ge}')
        # Extract frame
        frame_png = slice_path + '.frame.png'
        if os.path.exists(slice_path) and FFMPEG_AVAILABLE:
            try:
                ff_cmd = [FFMPEG_PATH, '-y', '-ss', str(ts_float), '-i', slice_path, '-vframes', '1', frame_png]
                subprocess.run(ff_cmd, capture_output=True, text=True, timeout=30)
            except Exception as fe:
                base_errors.append(f'frame-fail:{ordx}:{fe}')
        # Open image (may fail)
        im = None
        try:
        ...
        if im is None:
            # Create placeholder
            im = Image.new('RGB', (160, 90), (40, 40, 40))
        # Scale slice to target height
        if target_h > 0:
            w, h = im.size
            if h != target_h:
                new_w = max(2, int(w * (target_h / float(h))))
                im = im.resize((new_w, target_h))
        # Draw index overlay (simple)
        try:
            import PIL.ImageDraw as ImageDraw  # type: ignore
            draw = ImageDraw.Draw(im)
            draw.rectangle([(0,0),(40,20)], fill=(0,0,0))
            draw.text((4,4), str(ordx), fill=(255,255,0))
        except Exception:
            pass
        images.append(im)
    ...
    max_h = max(i.size[1] for i in images) if images else target_h
    mosaic = Image.new('RGB', (total_w, max_h), (15,15,15))
    off_x = 0
    for im in images:
        mosaic.paste(im, (off_x, 0))
        off_x += im.size[0]
    ...
    try:
        resp.headers['X-Slice-Count'] = str(slice_count)
        resp.headers['X-Mosaic-Width'] = str(total_w)
        if base_errors:
            resp.headers['X-Mosaic-Errors'] = ';'.join(base_errors[:5])
    except Exception:
        pass
    return resp

@app.route('/api/warm_slices')
def api_warm_slices():
    """Pre-generate (warm) one or more slice orders for a single ultra-wide video.
    Query params:
      user_code     : 4-digit pairing code (or active session)
      video_path    : relative video path within uploads (required)
      slice_mode    : split-h (default) or split-v
      slice_count   : integer >1
      orders        : comma list of orders (e.g. 0,1,2) or 'all' (default all 0..count-1)
      refresh       : if '1' force regenerate even if cached
    Returns JSON: { success, video_path, slice_mode, slice_count, results:[{order,status,size_bytes}] }
    """
    header_code = request.headers.get('X-User-Code') or request.args.get('user_code')
    user_key = _resolve_user_key_by_code(header_code)
    if not user_key and not _safe_user_key():
        return {'success': False, 'error': 'pair code required'}, 403
    vpath = request.args.get('video_path') or request.args.get('file')
    if not vpath:
        return {'success': False, 'error': 'video_path required'}, 400
    mode = (request.args.get('slice_mode') or 'split-h').lower()
    try:
        scount = int(request.args.get('slice_count', 1))
    except Exception:
        scount = 1
    if scount < 2:
        return {'success': False, 'error': 'slice_count must be >1'}, 400
    orders_raw = (request.args.get('orders') or 'all').strip().lower()
    if orders_raw in ('all','*'):
        orders = list(range(scount))
    else:
        tmp = []
        for p in orders_raw.split(','):
            p = p.strip()
            if not p: continue
            try:
                iv = int(p)
                ...
                pass
        orders = sorted(set(tmp))
    if not orders:
        return {'success': False, 'error': 'no valid orders specified'}, 400
    refresh = (request.args.get('refresh','0').lower() in ('1','true','yes','force'))
    results = []
    # Resolve original video path (local only; if R2 enabled we delegate to slice_video lazily)
    if r2_enabled():
        # We simply hit the slice endpoint HEAD to trigger CDN download path like a client
        base_url = request.host_url.rstrip('/')
        for o in orders:
            slice_url = url_for('slice_video', video_path=vpath, _external=True)
            slice_url += f"?slice_mode={mode}&slice_count={scount}&slice_order={o}"
            try:
                import urllib.request
                req = urllib.request.Request(slice_url, method='HEAD')
                urllib.request.urlopen(req, timeout=20)
                results.append({'order': o, 'status': 'ok-proxied'})
            except Exception as e:
                results.append({'order': o, 'status': 'error', 'error': str(e)})
        return {'success': True, 'video_path': vpath, 'slice_mode': mode, 'slice_count': scount, 'results': results}
    # Local path
    orig_path = os.path.join(app.config['UPLOAD_FOLDER'], vpath)
    if not os.path.exists(orig_path):
        return {'success': False, 'error': 'original video not found', 'video_path': vpath}, 404
    os.makedirs(SLICE_CACHE_FOLDER, exist_ok=True)
    for o in orders:
        cache_key = f"{SLICE_ENCODER_VERSION}__{vpath.replace('/', '_').replace('\\', '_')}_slice_{mode}_{scount}_{o}"
        slice_path = os.path.join(SLICE_CACHE_FOLDER, f"{cache_key}.mp4")
        existed = os.path.exists(slice_path)
        ...
        if not existed:
            ok = generate_video_slice_ffmpeg(orig_path, slice_path, mode, scount, o)
            if not ok or not os.path.exists(slice_path):
                results.append({'order': o, 'status': 'error', 'error': 'generation-failed'})
                continue
        try:
            sizeb = os.path.getsize(slice_path)
        except Exception:
            sizeb = None
        results.append({'order': o, 'status': status, 'size_bytes': sizeb})
    return {'success': True, 'video_path': vpath, 'slice_mode': mode, 'slice_count': scount, 'results': results}

@app.route('/api/warm_playlist_slices/<store_id>/<screen_id>')
def api_warm_playlist_slices(store_id, screen_id):
    """Warm all slice orders for every sliced video present in a playlist screen.
    Query params:
      user_code  : pairing code
      include_all_orders=1 : warm all orders 0..count-1 (default just this screen's order)
      refresh=1 : force regeneration
    """
    header_code = request.headers.get('X-User-Code') or request.args.get('user_code')
    user_key = _resolve_user_key_by_code(header_code)
    if not user_key and not _safe_user_key():
    ...
    ukey = user_key or _safe_user_key()
    cfg = ensure_playlists_structure(load_store_config_for_user_safe_key(ukey) if ukey else load_store_config())
    screens = (cfg.get('screens') or {}).get(store_id)
    if not isinstance(screens, dict):
        return {'success': False, 'error': 'store not found'}, 404
    screen = screens.get(screen_id)
    if not screen:
        # attempt legacy id mapping similar to playlist endpoint
        if '_' in screen_id:
            short = screen_id.split('_',1)[1]
            screen = screens.get(short)
        if not screen:
            return {'success': False, 'error': 'screen not found'}, 404
    include_all = (request.args.get('include_all_orders','0').lower() in ('1','true','yes','on'))
    refresh = (request.args.get('refresh','0').lower() in ('1','true','yes','force'))
    # Determine this screen's order if part of a sync group
    screen_order_hint = None
    try:
        # Look at first video with sync_ref to infer order
        for it in (screen.get('playlist') or []):
            if isinstance(it, dict) and it.get('sync_ref') and isinstance(it.get('sync_ref'), dict):
                try:
                    screen_order_hint = int(it['sync_ref'].get('order'))
                    break
                except Exception:
                    pass
    except Exception:
    ...
    for it in (screen.get('playlist') or []):
        if not isinstance(it, dict):
            continue
        if (it.get('media_type') or classify_media(it.get('file') or '')) != 'video':
            continue
        sref = it.get('sync_ref') if isinstance(it.get('sync_ref'), dict) else None
        if not sref:
            continue
        ...
        if include_all:
            ...
            orders = []
            if screen_order_hint is not None:
                orders.append(screen_order_hint)
            else:
                try:
                    orders.append(int(sref.get('order') or 0))
                except Exception:
                    orders.append(0)
        # Call internal warm logic per order
        item_results = []
        for o in orders:
            cache_key = f"{SLICE_ENCODER_VERSION}__{vpath.replace('/', '_').replace('\\', '_')}_slice_{mode}_{count}_{o}"
            slice_path = os.path.join(SLICE_CACHE_FOLDER, f"{cache_key}.mp4")
            existed = os.path.exists(slice_path)
            if existed and refresh:
                try:
            ...
            if not existed:
                orig_path = os.path.join(app.config['UPLOAD_FOLDER'], vpath)
                if not os.path.exists(orig_path):
                    item_results.append({'order': o, 'status': 'error', 'error': 'source-missing'})
                    continue
                ok = generate_video_slice_ffmpeg(orig_path, slice_path, mode, count, o)
                if not ok or not os.path.exists(slice_path):
                    item_results.append({'order': o, 'status': 'error', 'error': 'generation-failed'})
                    continue
            try:
                sizeb = os.path.getsize(slice_path)
            except Exception:
                sizeb = None
            item_results.append({'order': o, 'status': status, 'size_bytes': sizeb})
        warmed.append({'file': vpath, 'mode': mode, 'count': count, 'orders': item_results})
    return {'success': True, 'store_id': store_id, 'screen_id': screen_id, 'warmed': warmed, 'include_all_orders': include_all}
...
def generate_video_slice_ffmpeg(input_path, output_path, mode, count, order):
    ...
    try:
        ...
        ffprobe_cmd = FFPROBE_PATH or (FFMPEG_PATH.replace('ffmpeg', 'ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe')
        probe_cmd = [
        ...
        # Shared base encoding arguments for video when source already has audio
        # Industrial tuning additions:
        # -movflags +faststart : moov atom at head for faster playback start
        # -tune zerolatency    : reduce decoder delay (important for sync start)
        # -fflags +genpts      : generate missing pts if irregular
        # -avoid_negative_ts make_zero : consistent pts origin
        # -max_muxing_queue_size 1024  : avoid queue errors on some inputs
        # -x264-params (if supported)  : force scenecut off (already sc_threshold=0) and repeat headers
        ffmpeg_cmd = [
            FFMPEG_PATH, '-y',
            '-i', input_path,
            '-vf', crop_filter,
        ...
        if has_audio:
            ffmpeg_cmd += [
                '-c:a', 'aac', '-b:a', '128k', '-ac', '2', '-ar', '48000', '-map', '0:a:0?'
            ]
        else:
            # Inject silent audio to prevent black screen on some Android TVs when no audio track exists.
            ffmpeg_cmd = [
                FFMPEG_PATH, '-y',
                '-i', input_path,
                '-f', 'lavfi', '-i', 'anullsrc=channel_layout=stereo:sample_rate=48000',
                '-filter_complex', f"[0:v:0]{crop_filter}[v]",
                '-map', '[v]', '-map', '1:a:0',
                '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-profile:v', 'baseline', '-level', '4.0',
                '-preset', 'veryfast', '-tune', 'zerolatency', '-crf', '20', '-g', str(gop), '-keyint_min', str(gop), '-sc_threshold', '0',
                '-vsync', 'cfr', '-r', str(max(10, fps)), '-force_key_frames', 'expr:gte(t,n_forced*1)',
                '-fflags', '+genpts', '-avoid_negative_ts', 'make_zero', '-movflags', '+faststart', '-max_muxing_queue_size', '1024',
        ...
        if result.returncode == 0:
            try:
                ...
                import json as _json
                meta = {
                    'encoder_version': SLICE_ENCODER_VERSION,
                    'generated_utc': datetime.now(timezone.utc).isoformat(),
                    'has_audio': True if has_audio or True else False,  # always True after silent inject
                    'crop': {'mode': mode, 'count': count, 'order': order, 'x': crop_x, 'y': crop_y, 'w': slice_width, 'h': slice_height},
                    'source': {
                        'width': input_width,
                        'height': input_height,
                        'fps': fps
                ...
                print(f"WARNING: Failed to write slice metadata sidecar: {me}")
...
def ffmpeg_status():
    ...
    try:
        ...
        FFMPEG_AVAILABLE = check_ffmpeg_available()
        # Probe ffprobe again (lightweight)
        global FFPROBE_PATH
        ffprobe_available = False
        if FFMPEG_AVAILABLE:
            candidate = FFPROBE_PATH or (FFMPEG_PATH.replace('ffmpeg','ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe')
            try:
                pr = subprocess.run([candidate,'-version'], capture_output=True, text=True, timeout=5)
                if pr.returncode == 0:
        ...
        status = {
            ...
            'ffprobe_path': FFPROBE_PATH if ffprobe_available else None,
            'silent_audio_injection': True,  # Feature flag: we inject silent audio for video slices w/o audio
...
# --- Debug slice inspection (no generation) ---
@app.route('/api/debug_slice', methods=['GET'])
def api_debug_slice():
    """Return computed crop parameters for a given video path and slice params without producing a slice.
    Query:
      file=users/.../video.mp4 (relative path under uploads or R2 path)
      slice_mode=split-h|split-v (default split-h)
      slice_count=N (int)
      slice_order=K (0-based)
    """
    try:
        vpath = request.args.get('file') or ''
        mode = (request.args.get('slice_mode') or 'split-h').lower()
        count = int(request.args.get('slice_count') or 1)
        order = int(request.args.get('slice_order') or 0)
        if count < 1: count = 1
        if order < 0: order = 0
        notes = []
        local_path = os.path.join(app.config['UPLOAD_FOLDER'], vpath)
        if not os.path.exists(local_path) and r2_enabled():
            try:
                r2_download_to_local(vpath)
            except Exception as e:
                notes.append(f'restore_failed:{e}')
        if not os.path.exists(local_path):
            return jsonify({'success': False, 'error': 'file not found locally', 'file': vpath}), 404
        input_width = 0; input_height = 0; fps = 30; has_audio = False
        if FFMPEG_AVAILABLE:
            try:
                import subprocess, json
                ffprobe_cmd = FFMPEG_PATH.replace('ffmpeg','ffprobe') if 'ffmpeg' in FFMPEG_PATH else 'ffprobe'
                probe_cmd = [ffprobe_cmd,'-v','quiet','-print_format','json','-show_format','-show_streams',local_path]
                ...
                if pr.returncode == 0:
                    ...
                    for stream in pdata.get('streams', []):
                        ctype = stream.get('codec_type')
                        if ctype == 'video' and input_width == 0:
                            input_width = int(stream.get('width') or 0)
                            input_height = int(stream.get('height') or 0)
                            rfr = (stream.get('r_frame_rate') or '').strip()
                            try:
                                if rfr and '/' in rfr:
                                    num, den = rfr.split('/')
                                    num = float(num); den = float(den) if float(den) != 0 else 1
                                    val = num/den
                                    if 10 <= val <= 120:
                                        fps = int(round(val))
                            except Exception:
                    ...
                    notes.append(f'probe_fail:{pr.stderr[:80]}')
            except Exception as e:
                notes.append(f'probe_exc:{e}')
        if input_width == 0 or input_height == 0:
            input_width = 5760; input_height = 1080; notes.append('fallback_dimensions')
        def _even(x:int)->int: return x - (x % 2)
        if mode == 'split-h':
            w_per = input_width / max(1,count)
            left = int(round(order * w_per)); right = int(round((order+1)*w_per))
            crop_x = max(0, min(left, input_width-2)); crop_y = 0
            width = max(2, min(right-left, input_width-crop_x)); height = input_height
        elif mode == 'split-v':
            h_per = input_height / max(1,count)
            top = int(round(order * h_per)); bottom = int(round((order+1)*h_per))
            crop_y = max(0, min(top, input_height-2)); crop_x = 0
            height = max(2, min(bottom-top, input_height-crop_y)); width = input_width
        else:
            return jsonify({'success': False, 'error': 'unsupported mode'}), 400
        crop_x=_even(crop_x); crop_y=_even(crop_y)
        width=_even(min(width, input_width-crop_x)); height=_even(min(height, input_height-crop_y))
        if width < 2: width = 2
        if height < 2: height = 2
        return jsonify({
            'success': True,
            'file': vpath,
            'input': {'width': input_width, 'height': input_height, 'fps': fps, 'has_audio': has_audio},
            'derived': {'mode': mode, 'count': count, 'order': order},
